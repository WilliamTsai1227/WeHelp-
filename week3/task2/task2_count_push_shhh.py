...
while True:
        ...
        for title_div in title_divs:
            ...
            if a_tags: #如果a_tags不為空，(如果沒刪文)
                for a_tag in a_tags:
                    ...
                    div_push = href_soup.findAll('div', class_='push') #尋找push tag
                    for i in div_push: #找推tag
                        push_tag = i.find("span",class_="hl push-tag")
                        if push_tag:
                            if "推" in push_tag.text:
                                push += 1
                    div_shhh = href_soup.findAll('div', class_='push') #尋找push tag
                    for i in div_shhh: #找噓tag
                        shhh_tag = i.find("span",class_="f1 hl push-tag")
                        if shhh_tag:
                            if "噓" in shhh_tag.text:
                                shhh += 1
                    # text = 標題 , article_time = 發文日期  , push = 推次數  , shhh = 噓次數
                    push_and_shhh =(str(push)+"/"+str(shhh)) #數字轉換為文字data type在csv才不會用,間隔開 ex: 標題,10,/,0,發文日期
                    result.append([text,push_and_shhh,article_time])   
                    #print(text,push_and_shhh,article_time)
    #print(result)                
    #開始處理取得上一頁標籤網址
    previous_page = ""
    ...
    if page_num > max_page:
        break
#退出迴圈後將結果寫入csv
article_csv_file_path = 'article_push_shhh.csv'
